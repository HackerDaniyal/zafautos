...
import { uploadOrderDocument } from '@/server/actions/documentUpload';
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const uploadResult = await uploadOrderDocument(orderId, formData);
        if (uploadResult.success) {
          const docResult = await addOrderDocument(orderId, uploadResult.data.url);
          if (!docResult.success) {
            toast({ title: 'Error', description: docResult.error, variant: 'error' });
          }
        } else {
          toast({ title: 'Error', description: uploadResult.error, variant: 'error' });
        }
      }
