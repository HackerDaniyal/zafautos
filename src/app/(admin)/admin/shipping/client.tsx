...
export function ShippingClient() {
  ...
  const paramsRef = React.useRef<{
    page: number; pageSize: number; sortColumn: string;
    sortDirection: 'asc' | 'desc'; searchValue: string;
  ...
  const loadData = React.useCallback(async () => {
    ...
    try {
      ...
      const dateRange = p.filters.dateRange as { from?: string; to?: string } | undefined;
      ...
      const params: ShippingListParams = {
        ...
        status: (p.filters.status as ShipmentStatus) || undefined,
        carrier: (p.filters.carrier as string) || undefined,
    ...
    () => [
      {
        ...
        action: async (ids) => {
          ...
          if (result.success) { loadData(); loadStats(); } else { throw new Error(result.error); }
        },
      },
      {
