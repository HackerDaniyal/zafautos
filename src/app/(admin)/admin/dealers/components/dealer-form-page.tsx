...
export function DealerFormPage({ mode, initialData }: DealerFormPageProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'create') {
        toast({ title: 'Dealer created', variant: 'success' });
        router.push('/admin/dealers');
      } else {
        toast({ title: 'Dealer updated', variant: 'success' });
        router.push(`/admin/dealers/${initialData!.id}`);
      }
    } catch {
      toast({
        title: 'Error',
        description: `Failed to ${mode} dealer`,
        variant: 'error',
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={mode === 'create' ? 'Create Dealer' : 'Edit Dealer'}
        description={mode === 'create' ? 'Create a new dealer account' : `Editing dealer ${initialData?.email}`}
        action={{
