...
export class OrderRepository {
  ...
  async isVehicleReserved(vehicleId: string, excludeOrderId?: string): Promise<boolean> {
    const conditions = [
      eq(orders.vehicleId, vehicleId),
    ...
    const [result] = await this.orders.getClient()
  ...
  async getVehicleSnapshot(vehicleId: string) {
    const [vehicle] = await this.orders.getClient()
      .select({
        id: vehicles.id,
        vin: vehicles.vin,
  ...
  async getOrderWithRelations(orderId: string) {
    ...
    const invoicesList = await this.orders.getClient()
      .select()
      .from(invoices)
    ...
    // Fetch customer auth profile (firstName, lastName, phone, country) via users
    // Fetch profiles and vehicle images in parallel (each depends on its parent)
    const [customerAuthProfile, dealerProfile, vehicleImagesList] = await Promise.all([
      customerResult?.userId
        ? this.orders.getClient()
            .select()
            .from(profiles)
            .where(eq(profiles.userId, customerResult.userId))
            .limit(1)
            .then(async (rows) => {
              const prof = rows[0] ?? null;
              if (prof?.countryId) {
                const countryRow = await this.orders.getClient()
                  .select({ name: countries.name })
                  .from(countries)
                  .where(eq(countries.id, prof.countryId))
                  .limit(1)
                  .then((r) => r[0] ?? null);
                return { firstName: prof.firstName, lastName: prof.lastName, phone: prof.phone, country: countryRow?.name ?? null };
              }
              return { firstName: prof?.firstName ?? null, lastName: prof?.lastName ?? null, phone: prof?.phone ?? null, country: null };
            })
        : Promise.resolve(null),
      dealerResult
        ? this.orders.getClient()
    ...
    return {
      ...
      customer: customer ? { ...customer, profile: customerAuthProfile } : null,
