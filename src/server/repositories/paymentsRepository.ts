...
export class PaymentsRepository {
  ...
  async generateInvoiceNumber(): Promise<string> {
    ...
    const [{ maxNumber }] = await this.payments.getClient()
      .select({ maxNumber: sql<number>`coalesce(max(cast(substring(invoice_number from 10) as integer)), 0)` })
      .from(invoices)
      .where(sql`invoice_number like ${likePattern}`);
  ...
  async createInvoiceWithNumber(data: InferModel<typeof invoices, 'insert'>) {
    return this.payments.getClient().transaction(async (tx) => {
      const invoiceNumber = data.invoiceNumber ?? (() => {
        ...
        const year = new Date().getFullYear();
        const likePattern = `${prefix}-${year}-%`;
        return tx.select({ maxNumber: sql<number>`coalesce(max(cast(substring(invoice_number from 10) as integer)), 0)` })
          .from(invoices)
          .where(sql`invoice_number like ${likePattern}`)
