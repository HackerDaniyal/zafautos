...
export class VehicleRepository extends BaseRepository<typeof vehicles> {
  ...
  async listVehicles(options: VehicleListOptions = {}): Promise<PaginatedResult<typeof vehicles.$inferSelect>> {
    ...
    // Relation filters — support both single and array
    if (filters.manufacturerIds && filters.manufacturerIds.length > 0) conditions.push(inArray(vehicles.manufacturerId, filters.manufacturerIds));
    if (filters.modelIds && filters.modelIds.length > 0) conditions.push(inArray(vehicles.modelId, filters.modelIds));
    if (filters.bodyTypeIds && filters.bodyTypeIds.length > 0) conditions.push(inArray(vehicles.bodyTypeId, filters.bodyTypeIds));
    if (filters.fuelTypeIds && filters.fuelTypeIds.length > 0) conditions.push(inArray(vehicles.fuelTypeId, filters.fuelTypeIds));
    if (filters.transmissionIds && filters.transmissionIds.length > 0) conditions.push(inArray(vehicles.transmissionId, filters.transmissionIds));
    ...
    if (filters.countryIds && filters.countryIds.length > 0) conditions.push(inArray(vehicles.countryId, filters.countryIds));
    if (filters.destinationVehicleIds && filters.destinationVehicleIds.length > 0) conditions.push(inArray(vehicles.id, filters.destinationVehicleIds));
    ...
    if (filters.search) {
      ...
      // Also try exact year match if search is a 4-digit number
      const yearMatch = /^\d{4}$/.test(rawTerm);
      if (yearMatch) {
        const searchYear = parseInt(rawTerm, 10);
        ...
        // Split search terms for multi-word queries (e.g. "Toyota Corolla")
        const terms = rawTerm.split(/\s+/).filter(Boolean);
        const termConditions: SQL[] = terms.map((term) => {
          const termPattern = `%${term}%`;
          return or(
            like(vehicles.vin, termPattern),
            like(vehicles.stockNumber, termPattern),
            like(vehicles.auctionGrade, termPattern),
            like(vehicles.condition, termPattern),
          )!;
        });
        ...
        const matchingManufacturerIds = this.getClient()
          ...
          .from(manufacturers)
          .where(and(
            sql`${terms.map((t) => like(manufacturers.name, `%${t}%`)).reduce((a, b) => or(a, b)!)}`,
            sql`${manufacturers.deletedAt} IS NULL`,
          ));
        ...
        const matchingModelIds = this.getClient()
          ...
          .from(models)
          .where(and(
            sql`${terms.map((t) => like(models.name, `%${t}%`)).reduce((a, b) => or(a, b)!)}`,
            sql`${models.deletedAt} IS NULL`,
          ));
        ...
        // All terms must match something (AND across terms)
  ...
  async addImage(vehicleId: string, imageUrl: string, sortOrder: number = 0) {
    const [image] = await this.getClient()
      ...
      .values({ vehicleId, imageUrl, sortOrder, isPrimary: sortOrder === 0 })
      .returning();
    return image;
  }

  async addImageDirect(vehicleId: string, imageUrl: string, sortOrder: number, isPrimary: boolean) {
    const [image] = await this.getClient()
      .insert(vehicleImages)
      .values({ vehicleId, imageUrl, sortOrder, isPrimary })
      .returning();
    return image;
  }
  ...
  async listWithRelations(options: VehicleListOptions = {}): Promise<PaginatedResult<typeof vehicles.$inferSelect & { _manufacturerName: string | null; _modelName: string | null; _bodyTypeName: string | null; _fuelTypeName: string | null; _transmissionTypeName: string | null; _countryName: string | null }>> {
    ...
    const allImages = vehicleIds.length > 0
      ? await this.getClient()
          .select({ vehicleId: vehicleImages.vehicleId, imageUrl: vehicleImages.imageUrl, isPrimary: vehicleImages.isPrimary, sortOrder: vehicleImages.sortOrder })
          .from(vehicleImages)
    ...
    for (const img of allImages) {
      ...
      if (img.isPrimary) {
        primaryImageMap.set(img.vehicleId, img.imageUrl);
      }
    }
    for (const img of allImages) {
      if (!primaryImageMap.has(img.vehicleId)) {
        primaryImageMap.set(img.vehicleId, img.imageUrl);
      }
    }
    ...
    const enriched = result.data.map((v) => ({
      ...v,
      primaryImageUrl: resolveVehicleImageUrl(primaryImageMap.get(v.id) ?? null),
      _imageCount: imageCountMap.get(v.id) ?? 0,
  ...
  async countActiveVehiclesByEntity(entity: LookupEntity): Promise<{ id: string; count: number }[]> {
    const columnName = VehicleRepository.LOOKUP_COUNT_COLUMN_NAMES[entity];
    const col = sql`${sql.identifier(columnName)}`;
    const rows = await this.getClient()
      .select({ id: sql<string>`${col}`, count: sql<number>`count(*)::int` })
      ...
      .where(and(
        ...
        sql`${vehicles.status} = 'active'`
      ))
      .groupBy(col);
    return rows as { id: string; count: number }[];
  }

  async countVehiclesByCountry(): Promise<{ id: string; count: number }[]> {
    const rows = await this.getClient()
      .select({ id: sql<string>`${vehicles.countryId}`, count: sql<number>`count(*)::int` })
      .from(vehicles)
      .where(and(
        sql`${vehicles.countryId} IS NOT NULL`,
        sql`${vehicles.deletedAt} IS NULL`,
        sql`${vehicles.status} = 'active'`
      ))
      .groupBy(vehicles.countryId);
    return rows as { id: string; count: number }[];
  }

  async countActiveVehiclesByEntityFiltered(
    entity: LookupEntity | 'country',
    filters: VehicleFilterOptions,
  ): Promise<{ id: string; count: number }[]> {
    const columnName = entity === 'country'
      ? 'country_id'
      : VehicleRepository.LOOKUP_COUNT_COLUMN_NAMES[entity as LookupEntity];
    const col = sql`${sql.identifier(columnName)}`;

    const conditions: SQL[] = [];
    conditions.push(sql`${col} IS NOT NULL`);
    conditions.push(sql`${vehicles.deletedAt} IS NULL`);
    conditions.push(sql`${vehicles.status} = 'active'`);

    // Apply all filters except the entity we're counting
    if (filters.manufacturerIds && filters.manufacturerIds.length > 0) conditions.push(inArray(vehicles.manufacturerId, filters.manufacturerIds));
    if (filters.modelIds && filters.modelIds.length > 0) conditions.push(inArray(vehicles.modelId, filters.modelIds));
    if (filters.bodyTypeIds && filters.bodyTypeIds.length > 0) conditions.push(inArray(vehicles.bodyTypeId, filters.bodyTypeIds));
    if (filters.fuelTypeIds && filters.fuelTypeIds.length > 0) conditions.push(inArray(vehicles.fuelTypeId, filters.fuelTypeIds));
  ...
  async countActiveModelsFiltered(
    manufacturerIds: string[],
    filters: VehicleFilterOptions,
  ): Promise<{ id: string; count: number }[]> {
    const conditions: SQL[] = [];
    conditions.push(sql`${models.deletedAt} IS NULL`);
    conditions.push(sql`${vehicles.deletedAt} IS NULL`);
    conditions.push(sql`${vehicles.status} = 'active'`);

    // Make→Model dependency: if manufacturer IDs provided, restrict to those
    if (manufacturerIds.length > 0) {
      conditions.push(inArray(models.manufacturerId, manufacturerIds));
    }
    ...
    const rows = await this.getClient()
      .select({ id: sql<string>`${models.id}`, count: sql<number>`count(${vehicles.id})::int` })
      .from(models)
  ...
  async countVehiclesByManufacturerPublic(): Promise<{ id: string; count: number }[]> {
    const rows = await this.getClient()
      .select({ id: sql<string>`${vehicles.manufacturerId}`, count: sql<number>`count(*)::int` })
      .from(vehicles)
      .where(and(
        sql`${vehicles.manufacturerId} IS NOT NULL`,
        sql`${vehicles.deletedAt} IS NULL`,
        sql`${vehicles.status} = 'active'`
      ))
  ...
  async createColor(data: { name: string; hexCode?: string | null }) {
    const [result] = await this.getClient().insert(colors).values(data).returning();
    return result;
  }

  async updateColor(id: string, data: { name?: string; hexCode?: string | null }) {
  ...
  async normalizeVehicleImages(vehicleId: string) {
    const images = await this.getClient()
      .select()
      .from(vehicleImages)
      .where(eq(vehicleImages.vehicleId, vehicleId))
    ...
    if (primaryImages.length > 1) {
      ...
      for (let i = 1; i < primaryImages.length; i++) {
        await this.getClient()
          .update(vehicleImages)
          .set({ isPrimary: false })
          .where(eq(vehicleImages.id, primaryImages[i].id));
      }
    }
    ...
    if (primaryImages.length === 0 && images.length > 0) {
      ...
      await this.getClient()
        .update(vehicleImages)
        .set({ isPrimary: true })
        .where(eq(vehicleImages.id, images[0].id));
    }

    const normalized = await this.getClient()
      .select()
      .from(vehicleImages)
      .where(eq(vehicleImages.vehicleId, vehicleId))
      .orderBy(asc(vehicleImages.sortOrder));
    ...
    for (const img of normalized) {
      const expected = prevSort + 1;
      if (img.sortOrder !== expected) {
        needsUpdate = true;
        await this.getClient()
          .update(vehicleImages)
          .set({ sortOrder: expected })
