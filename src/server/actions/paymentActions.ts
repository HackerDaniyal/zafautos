...
export async function createInvoice(
  ...
  try {
    ...
    // Check if there's already an active (non-deleted) invoice for this order
    const existingInvoices = await paymentService.getInvoicesByOrderId(validated.orderId);
    const activeInvoice = existingInvoices.find(inv => inv.status !== 'cancelled' && !inv.deletedAt);
    if (activeInvoice) {
      return { success: false, error: 'An active invoice already exists for this order. Cancel or delete the existing invoice first.', code: 'INVOICE_EXISTS' };
    }
...
export async function changePaymentStatus(
  ...
  try {
    const auth = await requireAuth();
    await requirePermission(auth, 'payments.update');
...
export async function deletePayment(paymentId: string): Promise<ActionResult> {
  try {
    const auth = await requireAuth();
    await requirePermission(auth, 'payments.delete');
