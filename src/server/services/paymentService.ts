...
export class PaymentService {
  ...
  async changePaymentStatus(paymentId: string, newStatus: string, userId?: string, note?: string) {
    const payment = await this.paymentsRepo.payments.findById(paymentId) as unknown as { id: string; status: string; orderId?: string } | null;
    if (!payment) {
      throw new PaymentNotFoundError(paymentId);
    }
    ...
    try {
      const statusMessages: Record<string, { title: string; body: string; type: string }> = {
        paid: { title: 'Payment Received', body: 'Your payment has been received and confirmed.', type: 'payment.received' },
        failed: { title: 'Payment Failed', body: 'Your payment could not be processed.', type: 'payment.failed' },
        refunded: { title: 'Payment Refunded', body: 'Your payment has been refunded.', type: 'payment.refunded' },
      };
      const msg = statusMessages[newStatus];
      if (msg && payment.orderId) {
        const { orders } = await import('@/server/db/schema');
        const { db } = await import('@/server/db/client');
        const { eq } = await import('drizzle-orm');
        const [order] = await db.select({ customerId: orders.customerId }).from(orders).where(eq(orders.id, payment.orderId)).limit(1);
        if (order?.customerId) {
          await notificationService.dispatch({
            ...
            metadata: { paymentId, orderId: payment.orderId, status: newStatus },
            eventKey: `payment:${paymentId}:status:${newStatus}`,
          });
        }
      }
    } catch (err) {
      console.error('Payment notification dispatch failed (non-blocking):', err);
    }
