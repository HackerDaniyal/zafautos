...
import { newCmsRepository, type PaginationParams } from '@/server/repositories/newCmsRepository';
...
export class CmsService {
  ...
  async listBanners(params: PaginationParams) {
    return newCmsRepository.findBanners(params);
  }

  async getBanner(id: string) {
    const banner = await newCmsRepository.findBannerById(id);
    if (!banner) throw new CmsPageNotFoundError(id);
    return banner;
  }

  async getActiveBanners(placement?: string) {
    return newCmsRepository.findActiveBanners(placement);
  }

  async createBanner(data: any, createdBy: string) {
    return newCmsRepository.createBanner({ ...data, createdBy });
  }

  async updateBanner(id: string, data: any, updatedBy: string) {
    await this.getBanner(id);
    return newCmsRepository.updateBanner(id, { ...data, updatedBy });
  }

  async deleteBanner(id: string, deletedBy: string) {
    await this.getBanner(id);
    await newCmsRepository.softDeleteBanner(id, deletedBy);
  }

  // ── Testimonials ───────────────────────────────────────────────────────────

  async listTestimonials(params: PaginationParams) {
    return newCmsRepository.findTestimonials(params);
  }

  async getTestimonial(id: string) {
    const testimonial = await newCmsRepository.findTestimonialById(id);
    if (!testimonial) throw new CmsPageNotFoundError(id);
    return testimonial;
  }

  async getPublishedTestimonials() {
    return newCmsRepository.findPublishedTestimonials();
  }

  async createTestimonial(data: any, createdBy: string) {
    return newCmsRepository.createTestimonial({ ...data, createdBy });
  }

  async updateTestimonial(id: string, data: any, updatedBy: string) {
    await this.getTestimonial(id);
    return newCmsRepository.updateTestimonial(id, { ...data, updatedBy });
  }

  async deleteTestimonial(id: string, deletedBy: string) {
    await this.getTestimonial(id);
    await newCmsRepository.softDeleteTestimonial(id, deletedBy);
  }

  // ── FAQs ───────────────────────────────────────────────────────────────────

  async listFaqs(params: PaginationParams) {
    return newCmsRepository.findFaqs(params);
  }

  async getFaq(id: string) {
    const faq = await newCmsRepository.findFaqById(id);
    if (!faq) throw new CmsPageNotFoundError(id);
    return faq;
  }

  async getPublishedFaqs() {
    return newCmsRepository.findPublishedFaqs();
  }

  async createFaq(data: any, createdBy: string) {
    return newCmsRepository.createFaq({ ...data, createdBy });
  }

  async updateFaq(id: string, data: any, updatedBy: string) {
    await this.getFaq(id);
    return newCmsRepository.updateFaq(id, { ...data, updatedBy });
  }

  async deleteFaq(id: string, deletedBy: string) {
    await this.getFaq(id);
    await newCmsRepository.softDeleteFaq(id, deletedBy);
  }

  // ── Blog Posts ─────────────────────────────────────────────────────────────

  async listBlogPosts(params: PaginationParams) {
    return newCmsRepository.findBlogPosts(params);
  }

  async getBlogPost(id: string) {
    const post = await newCmsRepository.findBlogPostById(id);
    if (!post) throw new CmsPageNotFoundError(id);
    return post;
  }
  ...
  async getPublishedBlogPosts(params: PaginationParams) {
    return newCmsRepository.findPublishedBlogPosts(params);
  }

  async createBlogPost(data: any, createdBy: string) {
    const slug = data.slug || data.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-' + Date.now().toString(36);
    const existing = await newCmsRepository.findBlogPostBySlug(slug);
    if (existing) throw new ValidationError('A blog post with this slug already exists');
    return newCmsRepository.createBlogPost({ ...data, slug, createdBy, publishedAt: data.status === 'published' ? new Date() : undefined });
  }

  async updateBlogPost(id: string, data: any, updatedBy: string) {
    await this.getBlogPost(id);
    if (data.slug) {
      const existing = await newCmsRepository.findBlogPostBySlug(data.slug);
      if (existing && existing.id !== id) throw new ValidationError('A blog post with this slug already exists');
    }
    return newCmsRepository.updateBlogPost(id, { ...data, updatedBy, publishedAt: data.status === 'published' && !data.publishedAt ? new Date() : data.publishedAt });
  }

  async deleteBlogPost(id: string, deletedBy: string) {
    await this.getBlogPost(id);
    await newCmsRepository.softDeleteBlogPost(id, deletedBy);
  }

  // ── Media Uploads ──────────────────────────────────────────────────────────

  async listMediaUploads(params: PaginationParams) {
    return newCmsRepository.findMediaUploads(params);
  }

  async getMediaUpload(id: string) {
    const media = await newCmsRepository.findMediaUploadById(id);
    if (!media) throw new CmsPageNotFoundError(id);
    return media;
  }

  async createMediaUpload(data: any, uploadedBy: string) {
    return newCmsRepository.createMediaUpload({ ...data, uploadedBy });
  }

  async updateMediaUpload(id: string, data: any) {
    await this.getMediaUpload(id);
    return newCmsRepository.updateMediaUpload(id, data);
  }

  async deleteMediaUpload(id: string, deletedBy: string) {
    await this.getMediaUpload(id);
    await newCmsRepository.softDeleteMediaUpload(id, deletedBy);
  }

  // ── Active Homepage Sections (new) ─────────────────────────────────────────

  async getActiveHomepageSections() {
    return newCmsRepository.findHomepageSections();
  }

  async getAllHomepageSections() {
    return newCmsRepository.findAllHomepageSections();
  }
}
