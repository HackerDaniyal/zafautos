...
export class DashboardService {
  ...
  async getAnalyticsDashboard(dateFrom?: Date, dateTo?: Date) {
    const [
      ...
      // Extended analytics (A.15)
      dashboardRepo.getSupportStats(dateFrom, dateTo),
      dashboardRepo.getLeadStats(dateFrom, dateTo),
      dashboardRepo.getTopCustomers(dateFrom, dateTo),
      dashboardRepo.getWhatsAppClickStats(dateFrom, dateTo),
      dashboardRepo.getVehicleAging(),
    ]);
  ...
  async getAllTimeSummary() {
    return dashboardRepo.getAllTimeSummary();
  }
}
