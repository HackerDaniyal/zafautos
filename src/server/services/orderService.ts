...
import { orderNotes, dealers, orders } from '@/server/db/schema';
...
export class OrderService {
  ...
  /**
   * Updates an existing order's basic fields.
   */
  async updateOrder(orderId: string, data: { orderNumber?: string; customerId?: string | null; dealerId?: string | null; vehicleId?: string | null; status?: string; totalAmount?: number }) {
    if (!orderId) {
      throw new ValidationError('Order ID is required');
    }
    const existing = await this.orderRepo.orders.findById(orderId);
    if (!existing) {
      throw new OrderNotFoundError(orderId);
    }

    // Check if vehicle is already reserved by another active order
    if (data.vehicleId) {
      const isReserved = await this.orderRepo.isVehicleReserved(data.vehicleId, orderId);
      if (isReserved) {
        throw new ValidationError('This vehicle is already reserved by another active order');
      }
    }

    return this.orderRepo.orders.update(orderId, {
      ...data,
      updatedAt: new Date(),
    } as never);
  }
  ...
  async createOrder(data: CreateOrderDTO) {
    ...
    if (validatedData.vehicleId) {
      const isReserved = await this.orderRepo.isVehicleReserved(validatedData.vehicleId);
      if (isReserved) {
        throw new ValidationError('This vehicle is already reserved by another active order');
      }
    }
  ...
  async addOrderItem(data: AddOrderItemDTO) {
    ...
    // Snapshot vehicle data if vehicleId is provided
    let vehicleSnapshot = null;
    if (validatedData.vehicleId) {
      vehicleSnapshot = await this.orderRepo.getVehicleSnapshot(validatedData.vehicleId);
    }

    return this.orderRepo.addOrderItem({
      ...validatedData,
      vehicleSnapshot,
    } as unknown as Parameters<typeof this.orderRepo.addOrderItem>[0]);
  }
  ...
  async changeOrderStatus(
    ...
    // Dispatch notifications (non-blocking, errors caught inside)
    try {
      const order = existingOrder as unknown as { customerId?: string | null; orderNumber?: string };
      const statusMessages: Record<string, { title: string; body: string }> = {
        confirmed: { title: 'Order Confirmed', body: `Your order ${order.orderNumber ?? ''} has been confirmed.` },
        processing: { title: 'Order Processing', body: `Your order ${order.orderNumber ?? ''} is now being processed.` },
        shipped: { title: 'Order Shipped', body: `Your order ${order.orderNumber ?? ''} has been shipped.` },
      ...
      if (msg && order.customerId) {
        const [{ userId: custUserId }] = await db.select({ userId: orders.customerId }).from(orders).where(eq(orders.id, orderId)).limit(1);
        if (custUserId) {
          await notificationService.dispatch({
            ...
            metadata: { orderId, orderNumber: order.orderNumber, status: newStatus },
            eventKey: `order:${orderId}:status:${newStatus}`,
          });
        }
      }
    } catch (err) {
      console.error('Order notification dispatch failed (non-blocking):', err);
    }
