...
import { banners, blogPosts, cmsPages, cmsPageVersions, faqs, homepageSections, mediaUploads, menus, testimonials } from './cms';
...
import { emailLogs, messages, messageThreads, notificationPreferences, notifications } from './messages';
...
export const vehiclesRelations = relations(vehicles, ({ one, many }) => ({
  ...
  destinationCountries: many(vehicleDestinationCountries),
...
export const vehicleDestinationCountriesRelations = relations(vehicleDestinationCountries, ({ one }) => ({
  vehicle: one(vehicles, { fields: [vehicleDestinationCountries.vehicleId], references: [vehicles.id] }),
  country: one(countries, { fields: [vehicleDestinationCountries.countryId], references: [countries.id] }),
}));
...
export const countriesRelations = relations(countries, ({ one, many }) => ({
  ...
  vehicles: many(vehicles),
  destinationVehicles: many(vehicleDestinationCountries),
}));
...
export const bannersRelations = relations(banners, ({ one }) => ({
  createdByUser: one(users, { fields: [banners.createdBy], references: [users.id], relationName: 'banner_created_by' }),
  updatedByUser: one(users, { fields: [banners.updatedBy], references: [users.id], relationName: 'banner_updated_by' }),
}));

export const testimonialsRelations = relations(testimonials, ({ one }) => ({
  vehicle: one(vehicles, { fields: [testimonials.vehicleId], references: [vehicles.id] }),
  createdByUser: one(users, { fields: [testimonials.createdBy], references: [users.id], relationName: 'testimonial_created_by' }),
  updatedByUser: one(users, { fields: [testimonials.updatedBy], references: [users.id], relationName: 'testimonial_updated_by' }),
}));

export const faqsRelations = relations(faqs, ({ one }) => ({
  createdByUser: one(users, { fields: [faqs.createdBy], references: [users.id], relationName: 'faq_created_by' }),
  updatedByUser: one(users, { fields: [faqs.updatedBy], references: [users.id], relationName: 'faq_updated_by' }),
}));

export const blogPostsRelations = relations(blogPosts, ({ one }) => ({
  createdByUser: one(users, { fields: [blogPosts.createdBy], references: [users.id], relationName: 'blog_post_created_by' }),
  updatedByUser: one(users, { fields: [blogPosts.updatedBy], references: [users.id], relationName: 'blog_post_updated_by' }),
}));

export const mediaUploadsRelations = relations(mediaUploads, ({ one }) => ({
  uploadedByUser: one(users, { fields: [mediaUploads.uploadedBy], references: [users.id] }),
}));
