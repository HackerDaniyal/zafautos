...
export const vehicles = pgTable('vehicles', {
  ...
  statusFeaturedIdx: index('vehicles_status_featured_idx').on(table.status, table.isFeatured),
  statusManufacturerModelIdx: index('vehicles_status_manufacturer_model_idx').on(table.status, table.manufacturerId, table.modelId),
}));
...
export const vehicleDestinationCountries = pgTable('vehicle_destination_countries', {
  id: uuid('id').defaultRandom().primaryKey(),
  vehicleId: uuid('vehicle_id').references(() => vehicles.id, { onDelete: 'cascade' }).notNull(),
  countryId: uuid('country_id').references(() => countries.id, { onDelete: 'cascade' }).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
  vehicleIdx: index('vdc_vehicle_idx').on(table.vehicleId),
  countryIdx: index('vdc_country_idx').on(table.countryId),
  uniqueIdx: index('vdc_unique_idx').on(table.vehicleId, table.countryId),
}));
