...
async function seedReferenceData() {
  ...
  for (const c of COLORS) {
    const id = uuid();
    colorIds.push(id);
    await db.insert(schema.colors).values({ id, name: c.name, hexCode: c.hex }).onConflictDoNothing();
  }
...
async function seedMessagesNotifications(vehicleIds: string[], customerIds: string[], customerUserIds: string[]) {
  ...
  for (let i = 0; i < 20; i++) {
    await db.insert(schema.notifications).values({
      ...
      type: randomItem(['order.created', 'payment.received', 'shipping.updated', 'support.ticket_created', 'system.message']),
      category: randomItem(['order', 'payment', 'shipping', 'support', 'system']),
