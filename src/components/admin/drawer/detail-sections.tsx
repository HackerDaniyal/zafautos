...
function DetailGroup({ title, children, className, icon: Icon }: DetailGroupProps) {
  return (
    <div className={cn('space-y-3', className)}>
      <h4 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-ash">
        {Icon && <Icon className="size-3.5 text-ash" />}
        {title}
      </h4>
      ...
      {fields.map((field) => (
        <div key={field.label} className="space-y-1">
          <div className="flex items-center gap-1.5">
            {field.icon && (
              <span className="flex size-5 shrink-0 items-center justify-center rounded bg-signal-red/10">
                <field.icon className="size-3 text-signal-red" />
              </span>
            )}
