...
function buildFilterChips(filters: Partial<FilterState>) {
  ...
  (filters.models ?? []).forEach((v) => chips.push({ label: v, key: 'models', value: v }));
  ...
  (filters.countries ?? []).forEach((v) => chips.push({ label: v, key: 'countries', value: v }));
  ...
  return (
    <div className={cn('flex flex-wrap items-center gap-2', className)}>
      ...
      <Button
        ...
        size="sm"
        onClick={onClearAll}
        className="h-6 px-2 text-xs text-gray-500 hover:text-gray-900 hover:bg-transparent"
      >
        Clear all
      </Button>
    </div>
